...
"Partially automated" means rules check some aspects or identify candidates for human review.
Some rules return review findings only; a listed rule does not promise an automatic violation verdict.
"Manual review" means no active rule currently assists with that criterion. Human review is also
needed for partially automated criteria; no rule proves conformance.
